...
BYTE USB_init (VOID) {
	...
	//9)
	//Modified for captive bus. Enable only 1.8V LDO
#ifndef CAPTIVE_USB_MODE
	...
	// Modified for captive bus. No need to enable VBUS on interrupt.
#ifndef CAPTIVE_USB_MODE
...
BYTE USB_enable () {
	...
	//Modified for captive bus. VBUS off is not an error
#ifndef CAPTIVE_USB_MODE
	...
	USBKEYPID = 0x9628;                                                         //set KEY and PID to 0x9628 -> access to
	//configuration registers enabled
	/* TODO: Set this according to MCLK speed. */
...
BYTE USB_connectionInfo () {
	BYTE retVal = 0;
	//Modified for captive bus. Remove check for VBUS
#ifndef CAPTIVE_USB_MODE
...
#endif
#ifdef ENABLE_SELECTIVE_SUSPEND_PROPERTY
		if (bVal == 0xEE) {
			//P1OUT |= 0x10;
			usbClearOEP0ByteCount();
			wBytesRemainingOnIEP0 = SIZEOF_OS_DESCRIPTOR;
			usbSendDataPacketOnEP0 ( (PBYTE) &abromOSDescriptor);
		} else
#endif
		{
			bIndex = 0x00;
...
#ifdef ENABLE_SELECTIVE_SUSPEND_PROPERTY
BYTE usbGetExtPropOSDescriptor (VOID) {
	switch (tSetupPacket.wLength) {
	case SIZEOF_EXT_PROPERTIES_FULL:
	case SIZEOF_EXT_PROPERTIES_HEADER:
		usbClearOEP0ByteCount();
		wBytesRemainingOnIEP0 = tSetupPacket.wLength;
		usbSendDataPacketOnEP0 ( (PBYTE) &abromExtPropertiesDescriptor);
		break;
